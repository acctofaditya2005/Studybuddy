...
const Popup = () => {
  ...
  useEffect(() => {
    ...
    //arina: Load ElevenLabs settings
    chrome.storage.sync.get(['elevenLabsApiKey', 'elevenVoiceId'], (result) => {
      if (result.elevenLabsApiKey) {
        setElevenKey(result.elevenLabsApiKey);
      }
      if (result.elevenVoiceId) {
        setElevenVoiceId(result.elevenVoiceId);
      }
    });
  ...
  // arina: Save ElevenLabs settings
...
const handleReadSelection = async () => {
  ...
  try {
    // Step 1 — ask the active tab's content script for highlighted text
    ...
    try {
      ...
      setTtsStatus('error');
      setTtsError('Cannot access this page. Try a regular website.');
      return;
    }

    if (!selectedText) {
      setTtsStatus('error');
      setTtsError('No text selected — highlight some text on the page first.');
      return;
    }

    // Step 2 — send text to background ElevenLabs
    ...
    // Step 3 — build a data URL from the base64 MP3 and store it
    setAudioSrc(`data:audio/mpeg;base64,${result.base64Audio}`);
    setTtsStatus('ready');

  } catch (err) {
    setTtsStatus('error');
    setTtsError(err.message ?? 'Something went wrong.');
  }
};
        ...
        {activeTab === 'settings' && (
          <div>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              >
                {ttsStatus === 'loading' ? '⏳ Generating audio…' : '🔊 Read Selected Text'}
              </button>
              ...
              <p style={{ margin: '8px 0 0 0', fontSize: '12px', color: '#888', textAlign: 'center' }}>
                Highlight text on the page, then click above
              </p>
            </div>

            <hr style={{ margin: '4px 0 20px 0', border: 'none', borderTop: '1px solid #eee' }} />
            ...
            <div style={{ marginBottom: '15px' }}>
              ...
              <input
                ...
                placeholder="Paste your voice_id..."
                style={{
                  width: '100%',
                  padding: '10px',
                  border: '1px solid #ddd',
            ...
            >
              {saved ? '✓ Saved!' : 'Save ElevenLabs Settings'}
            </button>
